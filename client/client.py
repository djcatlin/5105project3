...
class StorageRegistry:
    ...
    def __init__(self, targets: list[str]) -> None:
        ...
        self._nodes   = {
            t: {"healthy": False, "stub": pb_grpc.StorageServiceStub(
                    grpc.insecure_channel(t))}
            for t in targets
        }
...
def replicate(method: str, request) -> None:
    ...
    threads = [threading.Thread(target=_send, args=(s,), daemon=True) for s in storage.backups()]
    for t in threads:
        t.start()
    for t in threads:
        t.join(timeout=3)
